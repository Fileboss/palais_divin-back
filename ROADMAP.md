...
- [ ] **I4.2 — `GET /api/v1/public/users/{userId}`** — single-user public profile returning `{id, displayName, createdAt}`. Public per the I2 catalog-default rule (display name isn't requester-scoped data). New `FindUserUseCase` in `user/domain/ports/`; `UserService.findById(UserId)` implementation; `PublicUserRestController` in `user/adapters/rest/`. 404 via existing `UserNotFoundException` → `GlobalExceptionHandler` (already wired by M7.1). `subject` (Keycloak UUID) and `email` stay private — never in response. No write paths, no outbox. Done when: GET returns 200 + the three-field body; 404 ProblemDetail on unknown UUID; 400 on malformed UUID; ArchUnit unchanged.
