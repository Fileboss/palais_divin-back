...
- [x] **I9.5 — Spec sync** — `OpenApiGenerationIT` now pins the full I9 surface: `contains("isFollowedByMe")` for I9.4, plus an explicit `get:` regex on `/api/v1/user/connections` symmetric with the existing I9.3 DELETE pin (I9.1/I9.2/I9.3 contracts were already pinned). Sync wire (`maven-antrun-plugin` `sync-openapi-to-frontend`, post-integration-test phase) was already in place — both back's `docs/openapi.yaml` and front's `doc/openapi.yaml` carry the regenerated I9 surface byte-identically. No new IT file, no `pom.xml` edit.

`MILESTONE I9` — `missing.md` third handoff closed.

- [x] **I9.6 — `labelI18n` on `TagSummary`** — I9.1 shipped `labelI18n: Map<String,String>` on the standalone `TagResponse`/`CreateTagRequest` but missed the embedded `TagSummary` (used by `RestaurantResponse.tags` on `GET /public/restaurants` and `GET /public/restaurants/{id}`), leaving tag chips on the FE catalog cards/filters French-only. Mirror is strictly additive: `TagSummary` record gains the field, `TagSummary.from(Tag)` passes `t.labelI18n()` through — domain `Tag` already carries it, JPA queries already select it, no migration / port / projector touched. `OpenApiGenerationIT` now pins `(?s)TagSummary:.*?labelI18n:` symmetric with I9.5's `get:`/`delete:` regexes. Empty maps still emit `{}` per the I9.1 wire convention.
